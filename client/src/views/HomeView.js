...
import { Eye, EyeOff, Plus, Info, ChevronsUpDown, CreditCard, MessageSquare, ArrowUpRight, ArrowDownLeft, ArrowLeftRight, ChevronRight, RefreshCw, PiggyBank, HelpCircle, Shirt, HandCoins, DollarSign, Edit3 } from 'lucide-react';
export const HomeView = ({ wallets, transactions, summary, onOpenAddModal, onNavigateTab, onEditTransaction, }) => {
                        ...
                        return (_jsxs("div", { onClick: () => {
                            ...
                            onNavigateTab('chat');
                                            ...
                                            return (_jsxs("div", { onClick: () => {
                                                ...
                                                }, className: "py-3 flex items-center justify-between hover:bg-white/5 active:bg-white/10 rounded-xl px-2.5 transition-all cursor-pointer group", children: [_jsxs("div", { className: "flex items-center gap-3 min-w-0 pr-4", children: [_jsx("div", { className: "w-10 h-10 rounded-2xl bg-[#233140] flex items-center justify-center shrink-0 shadow-sm group-hover:scale-105 transition-transform", children: getTxIcon(tx) }), _jsxs("div", { className: "min-w-0", children: [_jsx("h4", { className: "text-xs font-bold text-white truncate max-w-sm group-hover:text-[#29c184] transition-colors", children: tx.description }), _jsx("p", { className: "text-[11px] text-[#8b9aa8] truncate mt-0.5", children: tx.category_label || `${tx.category_name || 'Amaliyot'} • ${tx.wallet_name || 'Hamyon'}` })] })] }), _jsxs("div", { className: "flex items-center gap-2 shrink-0", children: [_jsxs("div", { className: "text-right", children: [_jsxs("p", { className: `text-xs font-black font-mono ${isExpense ? 'text-[#f0646e]' : 'text-[#23a887]'}`, children: [isExpense ? '-' : '+', tx.amount.toLocaleString('uz-UZ'), " UZS"] }), _jsx("p", { className: "text-[10px] text-[#8b9aa8] mt-0.5", children: tx.time_str || '' })] }), _jsx("div", { className: "w-7 h-7 rounded-lg bg-[#213040] opacity-0 group-hover:opacity-100 flex items-center justify-center text-[#8b9aa8] hover:text-white transition-all", children: _jsx(Edit3, { className: "w-3.5 h-3.5" }) })] })] }, tx.id));
