...
export const EditCategoryModal = ({ isOpen, onClose, category, onUpdate, onDelete }) => {
    if (!isOpen || !category)
        return null;
    ...
    return (_jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4", children: _jsxs("div", { className: "w-full max-w-md bg-[#19232e] rounded-3xl border border-[#354454] p-5 shadow-2xl space-y-4", children: [_jsxs("div", { className: "flex items-center justify-between pb-2 border-b border-[#354454]/60", children: [_jsx("h3", { className: "font-bold text-lg text-white", children: "Kategoriyani Tahrirlash" }), _jsx("button", { type: "button", onClick: () => {
                                triggerHaptic('light');
                                onClose();
                            }, className: "w-8 h-8 rounded-full bg-[#213040] flex items-center justify-center text-[#b6bfd0] hover:text-white", children: _jsx(X, { className: "w-4 h-4" }) })] }), _jsxs("form", { onSubmit: handleSave, className: "space-y-4", children: [_jsxs("div", { children: [_jsx("label", { className: "text-xs font-semibold text-[#8b9aa8]", children: "Kategoriya nomi" }), _jsx("input", { type: "text", required: true, value: name, onChange: (e) => setName(e.target.value), className: "w-full px-3.5 py-2.5 rounded-xl bg-[#213040] border border-[#354454] text-white text-sm focus:border-[#29c184] outline-none mt-1" })] }), _jsxs("div", { children: [_jsx("label", { className: "text-xs font-semibold text-[#8b9aa8]", children: "Oylik byudjet limiti (so'm)" }), _jsx("input", { type: "number", value: budgetLimitStr, onChange: (e) => setBudgetLimitStr(e.target.value), placeholder: "0 (cheklovsiz)", className: "w-full px-3.5 py-2.5 rounded-xl bg-[#213040] border border-[#354454] text-white text-sm focus:border-[#29c184] outline-none mt-1" })] }), _jsxs("div", { children: [_jsx("label", { className: "text-xs font-semibold text-[#8b9aa8]", children: "Ikonka" }), _jsx("div", { className: "grid grid-cols-7 gap-1.5 py-1.5 max-h-32 overflow-y-auto", children: icons.map((ic) => (_jsx("button", { type: "button", onClick: () => {
                                            triggerHaptic('light');
                                            setIcon(ic);
                                        }, className: `p-2 rounded-xl border flex items-center justify-center transition-all ${icon === ic ? 'bg-[#29c184] text-white border-[#29c184]' : 'bg-[#213040] text-[#899098] border-[#354454]'}`, children: _jsx(Icon, { name: ic, size: 16 }) }, ic))) })] }), _jsxs("div", { children: [_jsx("label", { className: "text-xs font-semibold text-[#8b9aa8]", children: "Rang" }), _jsx("div", { className: "flex gap-2 py-1 flex-wrap", children: colors.map((c) => (_jsx("button", { type: "button", onClick: () => {
                                            triggerHaptic('light');
                                            setColor(c);
                                        }, className: `w-7 h-7 rounded-full transition-transform ${color === c ? 'scale-125 ring-2 ring-white' : ''}`, style: { backgroundColor: c } }, c))) })] }), _jsxs("div", { className: "pt-2 flex items-center gap-3", children: [_jsxs("button", { type: "button", onClick: handleDelete, disabled: loading, className: `px-4 py-2.5 rounded-2xl font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer ${confirmDelete
